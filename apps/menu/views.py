...
from apps.menu.models import Category, Food, FoodPortion, Rating
...
class FoodViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        foods = (
            Food.objects.filter(foodportion__isnull=False).distinct()
            if category_id is None
            else Food.objects.filter(category_id=category_id, foodportion__isnull=False).distinct()
        )
...
class FoodPortionView(APIView):
    def get(self, request, *args, **kwargs):
        ...
        category_id = int(category_id) if category_id else None
        food_portions = (
            FoodPortion.objects.all()
            if category_id is None
            else FoodPortion.objects.filter(food__category_id=category_id)
        )
...
class PopularFoodsView(APIView):
    def get(self, request):
        ...
        food_counts = (
            OrderItemRecord.objects.values("food_portion__food_id")
